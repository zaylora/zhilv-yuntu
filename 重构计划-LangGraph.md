...
| RAG | query 改写 → 向量检索 → rerank，能力完整，但**仅覆盖 5 个硬编码城市**，无坐标 | `app/rag/` `app/agents/tools/rag_tool.py` → **本次重构移除** |
...
| 工具 | 高德 POI/周边/路线/天气、（可选）联网搜索/点评 | 复用 `map_service`/`weather_service`；候选全部来自实时 API |
| 流式进度 | LangGraph `astream` (`stream_mode="updates"`) → SSE | 前端展示"哪个 Agent 在跑" |
| 持久化 | 现有 SQLite + Redis 缓存 | 不变 |

> **决策：移除 RAG / 向量库**。设计图的检索通道全部基于实时 API（高德 POI / 联网搜索 / 高德天气），不含本地向量库；且 ④ 行程编排需要**真实坐标**做聚类排序，而 RAG 返回的攻略文本块无坐标、又只覆盖 5 个城市，泛化性差。因此**删除** `app/rag/`、`rag_tool.py`、ChromaDB、embedding、rerank、query 改写整条链路。本地体验性文案改由 ⑦ 汇总 Agent 用 LLM 自身知识 +（可选）联网搜索补足。
...
```
**移除依赖**：`chromadb`（连带 hnswlib/onnxruntime 等重依赖）；`langchain-community` 若仅 RAG 使用也可一并移除。embedding / rerank 模型不再需要。
...
- **实现**：1 次 LLM（结构化输出）。把用户意图改写成各通道的 POI 检索关键词（可借鉴原 `rag_tool.llm_rewrite_query` 的改写 prompt 思路，但不再做向量检索）。
- **降级**：LLM 不可用 → 规则改写（关键词模板拼装）。

### ③a spot_search_node —— 游玩地检索 Agent
- **职责**：产出**带坐标**的候选景点池，候选不足时**补搜**，有**退出条件**。
- **工具**：`map_service.search_places`（高德 POI，关键词/类目检索，返回坐标）+（可选）联网搜索补充热门景点。
...
- **工具**：高德 POI（餐饮类目，返回坐标/人均）+（可选）联网/点评搜索补充特色餐饮。
...
├── rag/                      # ❌ 整目录删除（retriever/vector_db）
├── agents/tools/rag_tool.py  # ❌ 删除
└── api/                      # 接口基本不变，新增 SSE 进度（见第六节）
```

> 旧 `trip_planner_agent.py` 的逻辑拆分迁移到 `nodes/` 后删除（其 token 提取、JSON 提取工具函数可抽到 `agents/llm_utils.py` 复用）。
> 同步清理：`scripts/` 下 RAG 相关脚本（`ingest_data.py`、`evaluate_rag_retrieval.py`、`debug_rag_retrieval.py`）、`eval/rag_eval_cases.json`、`tests/test_rag_retriever.py`、`backend/data/*.md` 与 `db/chroma_db/` 一并移除或归档。
...
**`TokenUsage` 重构**：现有字段偏 RAG 流水线（rewrite/embedding/rerank），随 RAG 移除而**废弃**。改为按 LLM 节点维度累加（仅 ②dispatch、⑦summarize，以及可选的 ③ 检索改写会真正消耗 token）：
```python
dispatch_*  spot_*  meal_*  transport_*  schedule_*  budget_*  summarize_*
```
或更简洁地保留 totals + 新增 `by_node: dict[str, NodeTokens]`。**保持 `total_*` 属性接口不变**，前端 token 统计页与 `GET /trip/stats` 不受影响（旧 rewrite/embedding/rerank 字段可保留为 0 以兼容历史数据）。
...
| **工具级** | 高德 POI/天气/路线、（可选）联网搜索、LLM 的调用次数、缓存命中、失败率、耗时 | 工具封装层计数器 + 日志 |
...
class RunMetrics(BaseModel):       # 整次编排汇总（随 Itinerary 一起可返回/入库）
    ...
    tool_calls: dict               # {amap_poi: n, amap_route: n, weather: n, web_search: n, llm: n}
...
- 在 `agents/tools/amap_tool.py`、`weather`、（可选）`search_tool` 封装层统一打点：调用计数、耗时、成功/失败、**缓存命中**（复用 `cache_service` 已有的 `cache hit/miss` 日志）。
...
| 处理 | 模块 |
| --- | --- |
| ✅ 直接复用 | `map_service`(geocode/search_places/estimate_route)、`cache_service`、`storage_service`、`export_service`、`config.py` |
| 🔧 小改复用 | `weather_service`(+季节降级)、`schemas`(加内部模型/扩展字段) |
| ♻️ 重写 | `trip_service.py`(变瘦封装)、`trip_planner_agent.py`(拆解后删除) |
| ❌ 删除 | `rag/`(retriever/vector_db)、`agents/tools/rag_tool.py`、RAG 相关脚本/测试/数据、ChromaDB 依赖 |
...
### M5 — 清理与文档（0.5d）
- 删除 `trip_planner_agent.py` 与**整个 RAG 链路**（`rag/`、`rag_tool.py`、相关脚本/测试/数据、`chromadb` 依赖、CHROMA/EMBEDDING/RERANK 配置）；更新 README/CHANGELOG；补 `.env.example`。
- ✅ 验收：`pip install` 后无 chromadb 等重依赖；全流程不依赖向量库仍可生成行程。
...
- **节点单测**：mock 工具（map/weather/搜索/LLM），验证每个节点输入→输出契约与降级路径。
...
**移除配置**：随 RAG 删除，`CHROMA_DB_DIR`、`CHROMA_COLLECTION_NAME`、`EMBEDDING_MODEL`、`EMBEDDING_BATCH_SIZE`、`RERANK_MODEL`、`REDIS_RAG_TTL_SECONDS`、`REDIS_RERANK_TTL_SECONDS` 一并从 `config.py` 与 `.env.example` 删除。
...
| ③a 游玩地检索（补搜/退出） | `spot_search_node` | 高德 POI（+可选联网搜索） |
